...
def main():
    ...
    while(True):
        ...
        for m in meterObjects:
            # sim_meter currently simulates meters and has case for physical meter in system
            # physical meter currently hardcoded to ID 6 (should be changed to improve implementation)
            # python socket programming to connect to local operations system database?
            if (m.id == 6):
                print("Meter " + str(m.id) + " is running physical meter process")
                #code for reading from Pi
                if (m.active == 'running'):
                    # physical meter is turned on
                    # request data from the LOS database
